...
async function redirect(ID) {
    ...
    if (list[ID]) {
        ...
        const spanname = document.createElement('span');
        spanname.id = "name";
        spanname.textContent = "deadcode";
        setTimeout(() => {
            document.querySelector("body").querySelector("span").parentNode.insertBefore(spanname, document.querySelector("body").querySelector("span").nextSibling); setOwO(12);
            setTimeout(() => {
                window.location.href = "https://deadcode.is-a.dev/";
            }, 1000);
        }, Math.random()*1500);
    }
}

function setOwO(predefinedOwO) {
    const owo = ["owo", "OwO", "oWo", "Ōwò", "uwu", "UwU", "uWu", "iwi", "qwq", "pwp", "TwT", "тwт", "TmT", "тmт", "ôWô", "òwó", "ùwú", ">w<", "-w-", ">m<", "omo", "OmO", "umu", "UmU", "-m-", "owø", "^w^"];
    document.querySelector("uwu").textContent = owo[predefinedOwO] || owo[Math.round(Math.random() * owo.length)];
}
redirect(getURL());